...
Table~\ref{tab:results} fixes the reporting format in advance and separates published evidence from our own measurements so that a reviewer never has to guess which is which: Block 1 reproduces values published by the source study on its own MPRINT setting~\cite{b1} (not our measurements, and not comparable metric-for-metric to Block 2); Block 2 is our own measurement, on a different substrate (the two public Bio-ML tasks), reported here in full rather than only in Section~\ref{sec:fullpilot}, since a single evaluation table a reviewer can check every claim against is more useful than a formally cleaner one left empty. Block 2 covers exactly what we actually ran: M1 and M1+M2 on the complete equivalence reference for both tasks (3{,}245 and 2{,}595 queries), and M1+M2+M3 and Full FedHarm on the 70-query-per-task pilot (140 queries total) described in Section~\ref{sec:fullpilot}. Nothing in that block is derived, projected, or estimated from another paper's numbers; every cell is either a direct measurement or ``---'' where the quantity is not defined at that operating point (for instance, F1 when zero decisions are committed). This paper's protocol also defines four MPRINT-specific ablations (B3--B6) and two operating-mode rows (supervised, autonomous); we have not run any of them, for the same reason given throughout this paper -- no MPRINT access (Section~\ref{sec:limitations}) -- and we no longer list them as empty or placeholder rows in this table, since Block 2 already reports the full extent of what this pipeline was actually run on.

\begin{table*}[t]
\caption{Evaluation table for FedHarm. Block 1 reproduces values published by the source study~\cite{b1} on its own MPRINT setting and is \emph{not} our measurement; ``---'' there means the source does not report that quantity. Block 2 is our own measurement, on Bio-ML rather than MPRINT (Section~\ref{sec:fullpilot}): M1 and M1+M2 rows use the full equivalence reference (3{,}245 NCIT--DOID / 2{,}595 OMIM--ORDO queries); M1+M2+M3 and Full FedHarm rows use a 70-query-per-task pilot (140 queries total). F1/FDR are decision accuracy / (1$-$accuracy) for the single-decision systems (M1, M1+M2, M1+M2+M3), and Coverage is 1.00 for those since none of them abstain. ``---'' in Block 2 means the quantity is undefined because zero decisions were committed at that operating point, not that it was left unmeasured. The MPRINT-specific ablations B3--B6 and the supervised/autonomous rows this table originally reserved space for are not reported here because we have no MPRINT access (Section~\ref{sec:limitations}); we do not list them as empty placeholder rows, since Block 2 already reports everything this pipeline was actually run on.}
\label{tab:results}
\centering
\scriptsize
...
\toprule
\textbf{Method} & \textbf{Task / cand. gen.} & \textbf{P} & \textbf{R} & \textbf{F1} & \textbf{Cov.} & \textbf{FDR} & \textbf{Reuse} \\
\midrule
\multicolumn{8}{@{}l}{\textit{Block 1 --- published, MPRINT setting~\cite{b1}, not our measurement, not comparable metric-for-metric to Block 2}} \\
B1 Retrieval only~\cite{b1}     & RAG, $k{=}3$      & --- & 0.98\textsuperscript{a} & --- & --- & --- & --- \\
B1 Retrieval only~\cite{b1}     & SNOMED pivot      & --- & 0.688\textsuperscript{b} & --- & --- & --- & --- \\
B2 Two-step pipeline~\cite{b1}  & RAG, $k{=}3$      & 0.78 & --- & --- & 1.00 & 0.22\textsuperscript{c} & 0.00 \\
B2 Two-step pipeline~\cite{b1}  & SNOMED pivot      & 0.91 & --- & --- & 1.00 & 0.09\textsuperscript{c} & 0.00 \\
B2 Two-step pipeline~\cite{b1}  & Unannotated, $k{=}3$ & 0.92 & --- & --- & 1.00 & 0.08\textsuperscript{c} & 0.00 \\
\midrule
\multicolumn{8}{@{}l}{\textit{Block 2 --- our measurements (Bio-ML, this work)}} \\
FedHarm-M1               & NCIT--DOID      & 0.769 & 0.765 & 0.766 & 1.00 & 0.234 & 0.00 \\
FedHarm-M1               & OMIM--ORDO      & 0.626 & 0.625 & 0.625 & 1.00 & 0.375 & 0.00 \\
FedHarm-M1+M2            & NCIT--DOID      & 0.769 & 0.765 & 0.766 & 1.00 & 0.234 & 0.00 \\
FedHarm-M1+M2            & OMIM--ORDO      & 0.626 & 0.625 & 0.625 & 1.00 & 0.375 & 0.00 \\
FedHarm-M1+M2+M3         & NCIT--DOID      & 0.314 & 0.314 & 0.314 & 1.00 & 0.686 & 0.00 \\
FedHarm-M1+M2+M3         & OMIM--ORDO      & 0.400 & 0.400 & 0.400 & 1.00 & 0.600 & 0.00 \\
Full FedHarm             & NCIT--DOID      & --- & --- & --- & 0.00\textsuperscript{d} & --- & 0.01\textsuperscript{e} \\
Full FedHarm             & OMIM--ORDO      & --- & --- & --- & 0.00\textsuperscript{d} & --- & 0.00\textsuperscript{e} \\
\bottomrule
\end{tabular}

\smallskip
{\footnotesize \textsuperscript{a}Ceiling the source reports would be attainable by expanding retrieval to $k{=}10$ (98\% of entries); it is not a measured end-to-end recall. \textsuperscript{b}Derived by us from a published count: 800 of 1{,}162 ICD-10 codes yielded at least one candidate. \textsuperscript{c}Empirical FDR is not reported by the source; this column instead shows $1-$precision as its precision is already an expert-agreement rate over 100\% coverage, so the two quantities coincide. \textsuperscript{d}Coverage at every tested $\epsilon \in \{0.05, 0.10, 0.20\}$; the Benjamini--Hochberg accepted set was empty at all three (Table~\ref{tab:m4}). \textsuperscript{e}Cumulative registry cache-hit rate by round three of the simulation (T6, Table~\ref{tab:targets}); registry \emph{size} itself still grows monotonically every round on both tasks (Section~\ref{sec:fullpilot}), this column reports reuse, not registry existence.}
\end{table*}
